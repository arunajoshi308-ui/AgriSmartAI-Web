...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex sticky top-0 z-50 bg-bento-bg/95 backdrop-blur-md border-b-2 border-bento-dark animate-fadeDown">
        <div className="max-w-6xl mx-auto w-full flex items-center justify-between px-6 py-3">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-full bg-bento-dark flex items-center justify-center transition-transform group-hover:rotate-12 group-hover:scale-110">
      ...
      {/* Mobile */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t-2 border-bento-dark flex justify-around items-center py-1 animate-fadeUp">
        {tabs.map((tab) => {
          const active = pathname === tab.href;
          return (
            <Link key={tab.href} href={tab.href} className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg transition-all active:scale-90 ${active ? "bg-bento-lime animate-pop" : ""}`}>
              <span className={`text-xl transition-transform ${active ? "scale-125 animate-float" : ""}`}>{tab.icon}</span>
