...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Top Bar — compact brand only */}
      <div className="md:hidden sticky top-0 z-50 bg-bento-bg/95 backdrop-blur-md border-b-2 border-bento-dark safe-top animate-fadeDown">
        <div className="flex items-center justify-center px-4 py-2.5">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-bento-dark flex items-center justify-center">
      ...
      {/* Mobile Bottom Nav — professional pill-style */}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className={`mobile-nav-item flex flex-col items-center justify-center gap-0.5 rounded-xl transition-all active:scale-90 ${
                  active ? "bg-bento-lime border-2 border-bento-dark animate-pop" : ""
                }`}
              >
