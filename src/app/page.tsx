      ...
      <div className="relative rounded-[20px] md:rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeDown" style={{ height: 360 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-6">
          ...
          <div className="hero-title-wrap">
            <h1
              className="text-2xl md:text-5xl font-black text-white text-center leading-tight animate-fadeUp delay-3"
              style={{ textShadow: "0 4px 24px rgba(28,28,22,0.95), 0 2px 4px rgba(28,28,22,0.85)" }}
            >
              AgriSmart AI 🌾
            </h1>
          </div>
          <p
            className="text-bento-lime font-bold text-xs md:text-lg mt-2 md:mt-3 text-center animate-fadeUp delay-4"
            style={{ textShadow: "0 2px 8px rgba(28,28,22,0.9)" }}
          >
      ...
      <div className="marquee bento-border bg-bento-dark rounded-2xl py-2.5 animate-fadeIn overflow-hidden">
        <div className="marquee-content">
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((text, i) => (
            <span key={i} className="text-bento-lime font-black text-xs md:text-sm whitespace-nowrap px-6">
              {text}
            </span>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {FEATURES.map((feat, i) => (
          <Link
            key={feat.href}
            href={feat.href}
            className={`bento-card ${feat.color} p-4 md:p-5 hover-lift animate-fadeUp press group relative overflow-hidden`}
        ...
        <div className="absolute inset-0 grid grid-cols-2 md:grid-cols-4 items-center px-4">
          {STATS.map((stat, i) => (
            <div key={stat.label} className="text-center">
              <div className="text-lg md:text-xl mb-1 animate-float" style={{ animationDelay: `${0.4 * (i + 1)}s` }}>{stat.icon}</div>
              <p className="text-xl md:text-3xl font-black text-bento-lime"
                style={{ textShadow: "0 2px 12px rgba(28,28,22,0.95)" }}
              >{stat.value}</p>
              <p className="text-[9px] md:text-[10px] font-bold text-white/70 uppercase mt-1"
                style={{ textShadow: "0 1px 4px rgba(28,28,22,0.85)" }}
              >{stat.label}</p>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 mt-3 md:mt-4 relative z-10">
          {["Gemini 3.5 Flash", "Disease Detection", "Yield Forecasting", "Soil Analysis"].map((tag, i) => (
            <span
              key={tag}
              className="bento-border rounded-xl px-2.5 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-black text-bento-dark hover-pop cursor-default"
      ...
      >
        ...
        <div className="relative z-10 p-4 md:p-7">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="font-black text-bento-lime text-lg md:text-xl"
              style={{ textShadow: "0 2px 12px rgba(28,28,22,0.95)" }}
            >Meet the Team</h2>
          </div>
          <p className="text-xs md:text-sm font-bold text-white/80 mb-3 md:mb-4"
            style={{ textShadow: "0 1px 6px rgba(28,28,22,0.85)" }}
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
            {TEAM.map((member, i) => (
              <div
                key={member.name}
                className="flex items-center gap-2 md:gap-3 bg-white/15 backdrop-blur-md border-2 border-bento-lime/40 rounded-2xl p-2.5 md:p-3 hover:border-bento-lime hover:bg-white/25 transition-all hover-lift"
                style={{
                  animation: visibleSections.has("team") ? `fadeUp 0.45s cubic-bezier(0.22,1,0.36,1) ${0.06 * (i + 1)}s both` : "none"
                }}
              >
                <div className={`${member.color} w-9 h-9 md:w-11 md:h-11 rounded-full border-2 border-bento-dark flex items-center justify-center text-base md:text-lg flex-shrink-0`}>
                  {member.emoji}
                </div>
