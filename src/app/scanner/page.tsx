  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4 page-enter">
      ...
      <div className="relative rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeDown" style={{ height: "110px" }}>
        <ProximityHover shape="circle" fill="stroke" strokeWidth={2} particleColor="#FFE0B2" gradientColor="#D1E67C" backgroundColor="#1C1C16" maxSize={28} minSize={4} gap={8} influence={200} autoPulse />
        ...
        <div className="absolute inset-0 flex items-center justify-between px-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-bento-peach text-base drop-shadow-lg" style={{ textShadow: "0 2px 8px rgba(28,28,22,0.9)" }}>Disease Scanner</h2>
              <p className="text-xs font-bold text-bento-lime" style={{ textShadow: "0 1px 6px rgba(28,28,22,0.8)" }}>AI Plant Disease Detection • Gemini Vision</p>
            </div>
          </div>
      ...
      {/* Upload */}
      <div className="bento-card bg-white p-6 cursor-pointer hover:bg-bento-bg hover-lift transition-all group animate-fadeUp" onClick={() => fileInputRef.current?.click()}>
        {selectedImage ? (
      ...
      {/* Crop Selector */}
      <div className="bento-card bg-bento-warm p-4 animate-fadeUp delay-2 hover-lift">
        <p className="text-sm font-black text-bento-dark mb-3">Select Crop Category:</p>
        <div className="flex flex-wrap gap-2">
          {CROPS.map((crop) => (
            <button key={crop} onClick={() => setCropHint(crop)} className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all hover:scale-105 active:scale-95 ${cropHint === crop ? "bg-bento-dark text-white animate-pop" : "bg-white bento-border text-bento-dark hover:bg-bento-lime"}`}>{crop}</button>
          ))}
        </div>
      </div>

      {/* Analyze */}
      <button onClick={(e) => { ripple(e); analyzeImage(); }} disabled={!selectedImage || loading} className="w-full bg-bento-lime bento-border rounded-2xl py-3.5 font-black text-sm text-bento-dark hover:scale-[1.01] active:scale-95 transition-all disabled:opacity-50 press animate-fadeUp delay-3 hover-lift ripple-container overflow-hidden relative">
        {loading ? (
        ...
        <div className="space-y-3">
          <div className="relative rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeUp">
            <div style={{ height: "45px" }}>
              <ProximityHover shape="hexagon" fill="solid" particleColor={diagnosis.healthStatus === "HEALTHY" ? "#D1E67C" : diagnosis.healthStatus === "DISEASED" ? "#FF6B6B" : "#FFE0B2"} backgroundColor="#1C1C16" maxSize={16} minSize={2} gap={8} influence={150} autoPulse />
            </div>
            <div className="bg-white p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`px-3 py-1 rounded-xl text-[10px] font-black animate-pop ${statusColors[diagnosis.healthStatus] || statusColors.WARNING}`}>{diagnosis.healthStatus}</span>
              </div>
              <div>
          ...
          {[
            ...
            <div key={card.title} className={`bento-card ${card.color} p-4 animate-fadeUp ${card.delay} hover-lift`}>
