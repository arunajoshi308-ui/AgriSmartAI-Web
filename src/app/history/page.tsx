...
export default function HistoryPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4 page-enter">
      {/* HEADER */}
      <div className="relative rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeDown" style={{ height: "100px" }}>
        <ProximityHover shape="triangle" fill="stroke" strokeWidth={2} particleColor="#E7E9D9" gradientColor="#D1E67C" backgroundColor="#1C1C16" maxSize={24} minSize={3} gap={8} influence={180} rotateOnHover autoPulse />
        ...
        <div className="absolute inset-0 flex items-center justify-between px-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-white text-base drop-shadow-lg" style={{ textShadow: "0 2px 8px rgba(28,28,22,0.9)" }}>Scan History & Lab</h2>
              <p className="text-xs font-bold text-bento-lime" style={{ textShadow: "0 1px 6px rgba(28,28,22,0.8)" }}>{history.length} saved {history.length === 1 ? "diagnosis" : "diagnoses"}</p>
            </div>
          </div>
          {history.length > 0 && (
            <button onClick={clearAll} className="pointer-events-auto text-xs font-black text-bento-lime hover:text-red-400 transition-colors hover:scale-105 active:scale-95 press">Clear All</button>
          )}
        </div>
      </div>
      ...
      {/* Empty State */}
      {history.length === 0 ? (
        <div className="relative rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeUp" style={{ height: "300px" }}>
          <ProximityHover shape="star" fill="solid" particleColor="#D1E67C" backgroundColor="#1C1C16" maxSize={22} minSize={3} gap={8} influence={200} autoPulse />
          <div className="absolute inset-0 bg-bento-dark/30 pointer-events-none" />
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-4">
            <div className="text-5xl mb-4 animate-float">🌱</div>
            <h3 className="font-black text-bento-lime text-lg mb-1 drop-shadow-lg" style={{ textShadow: "0 2px 12px rgba(28,28,22,0.9)" }}>No Scans Yet</h3>
            <p className="text-sm font-bold text-white/70 mb-4" style={{ textShadow: "0 1px 6px rgba(28,28,22,0.8)" }}>Start scanning plant leaves to build your history</p>
            <a href="/scanner" className="pointer-events-auto inline-block bg-bento-lime bento-border rounded-2xl px-6 py-3 font-black text-sm text-bento-dark hover:scale-105 active:scale-95 transition-all press hover-lift">📷 Open Disease Scanner</a>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          {history.map((record, i) => (
            <div key={i} className="bento-card bg-white overflow-hidden hover-lift animate-fadeUp" style={{ animationDelay: `${0.06 * (i + 1)}s` }}>
              <div className="p-4 cursor-pointer transition-all hover:bg-bento-bg/50" onClick={() => setExpanded(expanded === i ? null : i)}>
                ...
                {expanded === i && (
                  <div className="mt-3 pt-3 border-t-2 border-bento-dark/10 space-y-3 animate-fadeUp">
                    {[
                      ...
                      <div key={di} className="animate-fadeIn" style={{ animationDelay: `${0.08 * (di + 1)}s` }}>
