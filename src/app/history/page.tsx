    ...
    <div className="max-w-3xl mx-auto px-4 md:px-6 py-4 md:py-6 space-y-3 md:space-y-4 page-enter pb-24 md:pb-6">
      ...
      <div className="relative rounded-[20px] md:rounded-[28px] overflow-hidden border-2 border-bento-dark animate-fadeDown" style={{ height: 80 }}>
        ...
        <div className="absolute inset-0 flex items-center justify-between px-4 md:px-5">
          ...
          {history.length > 0 && (
            <button onClick={clearAll} className="pointer-events-auto text-xs font-black text-bento-lime hover:text-red-400 transition-colors hover:scale-105 active:scale-95 press mobile-touch px-2">Clear All</button>
          )}
        </div>
      </div>
          ...
          {history.map((record, i) => (
            <div key={i} className="bento-card bg-white overflow-hidden hover-lift animate-fadeUp relative" style={{ animationDelay: `${0.05 * (i + 1)}s` }}>
              <div className="p-3 md:p-4 cursor-pointer transition-all hover:bg-bento-bg/50" onClick={() => setExpanded(expanded === i ? null : i)}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2.5 md:gap-3 min-w-0">
                    {record.image && (
                      ...
                      <img src={record.image} alt={record.cropName} className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover bento-border transition-transform hover:scale-110 flex-shrink-0" />
                    )}
                ...
                {expanded === i && (
                  <div className="mt-2.5 md:mt-3 pt-2.5 md:pt-3 border-t-2 border-bento-dark/10 space-y-2.5 md:space-y-3 animate-fadeUp">
                    {[
                      ...
                      <div key={di} className="animate-fadeIn" style={{ animationDelay: `${0.06 * (di + 1)}s` }}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); deleteRecord(i); }} className="text-xs font-black text-red-500 hover:text-red-700 transition-colors hover:scale-105 active:scale-95 press mobile-touch">🗑️ Delete</button>
                  </div>
                )}
              </div>
            </div>
